...
use crate::{Audio::fill_buffer_callback, Controller::{
        ControllerData, ControllerOriginData, N64Controller, GCController
    }, Directory::{
        DirType
    }, Display::{
        AntiAlias, BitDepth, DisplayContext, Gamma, Resolution
    }, Exceptions::{
        Exception, ExceptionCode, ExceptionType, RegisterBlock
    }, GraphicsEngine::{
        RGBColor, Sprite
    }, Interrupt::{
        InterruptState
        ...
        GBCSupportType, GBCTitle, GameboyCartridgeHeader, NewTitle, OldTitle
    }};
...
#[inline(always)]
pub(crate) extern "C" fn C0_WRITE_COUNT(x: c_ulong) {
    unsafe { asm!("mtc0 {0},$9", in(reg) x); }
}

#[inline(always)]
pub(crate) extern "C" fn C0_COMPARE() -> c_ulong {
    let x: c_ulong;
...
#[inline(always)]
pub(crate) extern "C" fn C0_WRITE_COMPARE(x: c_ulong) {
    unsafe { asm!("mtc0 {0},$11", in(reg) x); }
}

#[inline(always)]
pub(crate) extern "C" fn C0_STATUS() -> c_ulong {
    let x: c_ulong;
...
#[inline(always)]
pub(crate) extern "C" fn C0_WRITE_STATUS(x: c_ulong) {
    unsafe { asm!("mtc0 {0},$12", in(reg) x); }
}

#[inline(always)]
pub(crate) extern "C" fn C0_READ_CR() -> c_ulong {
    let x: c_ulong;
...
#[inline(always)]
pub(crate) extern "C" fn C0_WRITE_CR(x: c_ulong) {
    unsafe { asm!("mtc0 {0},$13", in(reg) x); }
}

#[inline(always)]
pub(crate) extern "C" fn C0_READ_BADVADDR() -> c_ulong {
    let x: c_ulong;
...
#[inline(always)]
pub(crate) extern "C" fn C0_READ_EPC() -> c_ulong {
    let x: c_ulong;
    unsafe { asm!("mfc0 {0},$14", out(reg) x); }
    return x;
}
const C0_CAUSE_CE: c_int = 0x3000_0000;
const C0_CAUSE_EXC_CODE: c_int = 0x0000_007C;

#[inline(always)] pub(crate) extern "C" fn C0_GET_CAUSE_CE(cr: c_int) -> c_int { return ((cr) & C0_CAUSE_CE) >> 28; }
#[inline(always)] pub(crate) extern "C" fn C0_GET_CAUSE_EXC_CODE(sr: c_int) -> c_int { return ((sr) & C0_CAUSE_EXC_CODE) >> 2; }

/*
    cop1.h defines
 */
#[inline(always)]
pub(crate) extern "C" fn C1_FCR31() -> c_ulong {
    let x: c_ulong;
...
#[inline(always)]
pub(crate) extern "C" fn C1_WRITE_FCR31(x: *const c_ulong) {
    unsafe { asm!("ctc1 {0},$f31", in(reg) x.read_volatile()); }
}
...
#[inline(always)] pub(crate) extern "C" fn TICKS_READ() -> c_ulong { return C0_COUNT(); }

const TICKS_PER_SECOND: c_uint = 93750000 / 2;

#[inline(always)] pub(crate) extern "C" fn TICKS_DISTANCE(from: c_uint, to: c_uint) -> c_uint { return to - from; }
#[inline(always)] pub(crate) extern "C" fn TICKS_BEFORE(t1: c_uint, t2: c_uint) -> bool { return TICKS_DISTANCE(t1, t2) > 0; }
#[inline(always)] pub(crate) extern "C" fn TICKS_FROM_MS(val: c_uint) -> c_uint { return val * (TICKS_PER_SECOND / 1000); }
#[inline(always)] pub(crate) extern "C" fn get_ticks() -> c_ulong { return TICKS_READ(); }
#[inline(always)] pub(crate) extern "C" fn get_ticks_ms() -> c_ulong { return TICKS_READ() / ((TICKS_PER_SECOND / 1000) as c_ulong); }

/*
    timer.h defines
 */
#[inline(always)] pub(crate) extern "C" fn TIMER_TICKS(us: c_longlong) -> c_int { return (us * 46875 / 1000) as c_int; }
#[inline(always)] pub(crate) extern "C" fn TIMER_MICROS(tk: c_longlong) -> c_int { return (tk * 1000 / 46875) as c_int; }
...
extern "C" {
    ...
    pub(crate) fn controller_read_gc(data: *mut controller_data, rumble: *mut [c_uchar; 4]);
    ...
    pub(crate) fn read_mempak_address(controller: c_int, address: c_ushort, data: *mut c_uchar) -> c_int;

    // int write_mempak_address( int controller, uint16_t address, uint8_t *data );
    pub(crate) fn write_mempak_address(controller: c_int, address: c_ushort, data: *mut c_uchar) -> c_int;
    ...
    pub(crate) fn eeprom_read(block: c_int, buf: *const c_uchar);

    // void eeprom_write(int block, const uint8_t * const data);
    pub(crate) fn eeprom_write(block: c_int, data: *const c_uchar);

    /*
        mempak.h functions
     */
    // int read_mempak_sector( int controller, int sector, uint8_t *sector_data );
    pub(crate) fn read_mempak_sector(controller: c_int, sector: c_int, sector_data: *mut c_uchar) -> c_int;

    // int write_mempak_sector( int controller, int sector, uint8_t *sector_data );
    pub(crate) fn write_mempak_sector(controller: c_int, sector: c_int, sector_data: *mut c_uchar) -> c_int;
    ...
    pub(crate) fn read_mempak_entry_data(controller: c_int, entry: *mut entry_structure_t, data: *mut c_uchar) -> c_int;

    // int write_mempak_entry_data( int controller, entry_structure_t *entry, uint8_t *data );
    pub(crate) fn write_mempak_entry_data(controller: c_int, entry: *mut entry_structure_t, data: *mut c_uchar) -> c_int;
    ...
    pub(crate) fn tpak_set_value(controller: c_int, address: c_ushort, value: c_uchar) -> c_int;
    ...
    pub(crate) fn tpak_get_status(controller: c_int) -> c_uchar;
    ...
    pub(crate) fn tpak_write(controller: c_int, address: c_ushort, data: *mut c_uchar, size: c_ushort) -> c_int;

    // int tpak_read(int controller, uint16_t address, uint8_t* buffer, uint16_t size);
    pub(crate) fn tpak_read(controller: c_int, address: c_ushort, buffer: *mut c_uchar, size: c_ushort) -> c_int;

    /*
        display.h functions
     */
    // void display_init( resolution_t res, bitdepth_t bit, uint32_t num_buffers, gamma_t gamma, antialias_t aa );
    pub(crate) fn display_init(res: resolution_t, bit: bitdepth_t, num_buffers: c_uint, gamma: gamma_t, aa: antialias_t);
    ...
    pub(crate) fn io_read(pi_address: c_uint) -> c_uint;

    // void io_write(uint32_t pi_address, uint32_t data);
    pub(crate) fn io_write(pi_address: c_uint, data: c_uint);

    /*
        dragonfs.h functions
     */
    // int dfs_init(uint32_t base_fs_loc);
    pub(crate) fn dfs_init(base_fs_loc: c_uint) -> c_int;
    ...
    pub(crate) fn dfs_read(buf: *const c_void, size: c_int, count: c_int, handle: c_uint) -> c_int;

    // int dfs_seek(uint32_t handle, int offset, int origin);
    pub(crate) fn dfs_seek(handle: c_uint, offset: c_int, origin: c_int) -> c_int;

    // int dfs_tell(uint32_t handle);
    pub(crate) fn dfs_tell(handle: c_uint) -> c_int;

    // int dfs_close(uint32_t handle);
    pub(crate) fn dfs_close(handle: c_uint) -> c_int;

    // int dfs_eof(uint32_t handle);
    pub(crate) fn dfs_eof(handle: c_uint) -> c_int;

    // int dfs_size(uint32_t handle);
    pub(crate) fn dfs_size(handle: c_uint) -> c_int;

    /*
        graphics.h functions
     */
    // uint32_t graphics_make_color( int r, int g, int b, int a );
    pub(crate) fn graphics_make_color(r: c_int, g: c_int, b: c_int, a: c_int) -> c_uint;

    // uint32_t graphics_convert_color( color_t color );
    pub(crate) fn graphics_convert_color(color: color_t) -> c_uint;

    // void graphics_draw_pixel( display_context_t disp, int x, int y, uint32_t c );
    pub(crate) fn graphics_draw_pixel(disp: display_context_t, x: c_int, y: c_int, c: c_uint);

    // void graphics_draw_pixel_trans( display_context_t disp, int x, int y, uint32_t c );
    pub(crate) fn graphics_draw_pixel_trans(disp: display_context_t, x: c_int, y: c_int, c: c_uint);

    // void graphics_draw_line( display_context_t disp, int x0, int y0, int x1, int y1, uint32_t c );
    pub(crate) fn graphics_draw_line(disp: display_context_t, x0: c_int, y0: c_int, x1: c_int, y1: c_int, c: c_uint);

    // void graphics_draw_line_trans( display_context_t disp, int x0, int y0, int x1, int y1, uint32_t c );
    pub(crate) fn graphics_draw_line_trans(disp: display_context_t, x0: c_int, y0: c_int, x1: c_int, y1: c_int, c: c_uint);

    // void graphics_draw_box( display_context_t disp, int x, int y, int width, int height, uint32_t color );
    pub(crate) fn graphics_draw_box(disp: display_context_t, x: c_int, y: c_int, width: c_int, height: c_int, color: c_uint);

    // void graphics_draw_box_trans( display_context_t disp, int x, int y, int width, int height, uint32_t color );
    pub(crate) fn graphics_draw_box_trans(disp: display_context_t, x: c_int, y: c_int, width: c_int, height: c_int, color: c_uint);

    // void graphics_fill_screen( display_context_t disp, uint32_t c );
    pub(crate) fn graphics_fill_screen(disp: display_context_t, c: c_uint);

    // void graphics_set_color( uint32_t forecolor, uint32_t backcolor );
    pub(crate) fn graphics_set_color(forecolor: c_uint, backcolor: c_uint);
    ...
    pub(crate) fn register_AI_handler(callback: *mut extern fn());

    // void register_VI_handler( void (*callback)() );
    pub(crate) fn register_VI_handler(callback: *mut extern fn());

    // void register_PI_handler( void (*callback)() );
    pub(crate) fn register_PI_handler(callback: *mut extern fn());

    // void register_DP_handler( void (*callback)() );
    pub(crate) fn register_DP_handler(callback: *mut extern fn());

    // void register_TI_handler( void (*callback)() );
    pub(crate) fn register_TI_handler(callback: *mut extern fn());

    // void register_SI_handler( void (*callback)() );
    pub(crate) fn register_SI_handler(callback: *mut extern fn());

    // void register_SP_handler( void (*callback)() );
    pub(crate) fn register_SP_handler(callback: *mut extern fn());

    // void unregister_AI_handler( void (*callback)() );
    pub(crate) fn unregister_AI_handler(callback: *mut extern fn());

    // void unregister_VI_handler( void (*callback)() );
    pub(crate) fn unregister_VI_handler(callback: *mut extern fn());

    // void unregister_PI_handler( void (*callback)() );
    pub(crate) fn unregister_PI_handler(callback: *mut extern fn());

    // void unregister_DP_handler( void (*callback)() );
    pub(crate) fn unregister_DP_handler(callback: *mut extern fn());

    // void unregister_TI_handler( void (*callback)() );
    pub(crate) fn unregister_TI_handler(callback: *mut extern fn());

    // void unregister_SI_handler( void (*callback)() );
    pub(crate) fn unregister_SI_handler(callback: *mut extern fn());

    // void unregister_SP_handler( void (*callback)() );
    pub(crate) fn unregister_SP_handler(callback: *mut extern fn());
    ...
    pub(crate) fn data_cache_hit_invalidate(_: *mut c_void, _: c_ulong);

    // void data_cache_hit_writeback(volatile void *, unsigned long);
    pub(crate) fn data_cache_hit_writeback(_: *mut c_void, _: c_ulong);

    // void data_cache_hit_writeback_invalidate(volatile void *, unsigned long);
    pub(crate) fn data_cache_hit_writeback_invalidate(_: *mut c_void, _: c_ulong);

    // void data_cache_index_writeback_invalidate(volatile void *, unsigned long);
    pub(crate) fn data_cache_index_writeback_invalidate(_: *mut c_void, _: c_ulong);

    // void inst_cache_hit_writeback(volatile void *, unsigned long);
    pub(crate) fn inst_cache_hit_writeback(_: *mut c_void, _: c_ulong);

    // void inst_cache_hit_invalidate(volatile void *, unsigned long);
    pub(crate) fn inst_cache_hit_invalidate(_: *mut c_void, _: c_ulong);

    // void inst_cache_index_invalidate(volatile void *, unsigned long);
    pub(crate) fn inst_cache_index_invalidate(_: *mut c_void, _: c_ulong);
    ...
    pub(crate) fn rdp_set_clipping(tx: c_uint, ty: c_uint, bx: c_uint, by: c_uint);
    ...
    pub(crate) fn rdp_load_texture(textslot: c_uint, texloc: c_uint, mirror: mirror_t, sprite: *mut sprite_t) -> c_uint;

    // uint32_t rdp_load_texture_stride( uint32_t texslot, uint32_t texloc, mirror_t mirror, sprite_t *sprite, int offset );
    pub(crate) fn rdp_load_texture_stride(texslot: c_uint, texloc: c_uint, mirror: mirror_t, sprite: *mut sprite_t, offset: c_int) -> c_uint;

    // void rdp_draw_textured_rectangle( uint32_t texslot, int tx, int ty, int bx, int by,  mirror_t mirror );
    pub(crate) fn rdp_draw_textured_rectangle(textslot: c_uint, tx: c_int, ty: c_int, bx: c_int, by: c_int, mirror: mirror_t);
    ...
    pub(crate) fn rdp_draw_textured_rectangle_scaled(texslot: c_uint, tx: c_int, ty: c_int, bx: c_int, by: c_int, x_scale: c_double, y_scale: c_double, mirror: mirror_t);

    // void rdp_draw_sprite( uint32_t texslot, int x, int y ,  mirror_t mirror);
    pub(crate) fn rdp_draw_sprite(textslot: c_uint, x: c_int, y: c_int, mirror: mirror_t);

    // void rdp_draw_sprite_scaled( uint32_t texslot, int x, int y, double x_scale, double y_scale,  mirror_t mirror);
    pub(crate) fn rdp_draw_sprite_scaled(textslot: c_uint, x: c_int, y: c_int, x_scale: c_double, y_scale: c_double, mirror: mirror_t);

    // void rdp_set_primitive_color( uint32_t color );
    pub(crate) fn rdp_set_primitive_color(color: c_uint);

    // void rdp_set_blend_color( uint32_t color );
    pub(crate) fn rdp_set_blend_color(color: c_uint);
    ...
    pub(crate) fn new_timer(ticks: c_int, flags: c_int, callback: extern fn(ovfl: c_int)) -> *mut timer_link_t;

    // void start_timer(timer_link_t *timer, int ticks, int flags, void (*callback)(int ovfl));
    pub(crate) fn start_timer(timer: *mut timer_link_t, ticks: c_int, flags: c_int, callback: extern fn(ovfl: c_int));
    ...
    pub(crate) fn register_exception_handler(cb: extern fn(*mut exception_t));
